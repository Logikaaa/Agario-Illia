...
eats = [Eat(randint(-2000, 2000), randint(-2000, 2000), 10, randint(0, 255)) for _ in range(300)]
running = True
while running:
    for e in event.get():
        if e.type == QUIT:
    ...
    for eat in eats:
        if eat.check_collision(my_player[0], my_player[1], my_player[2]):
            ...
            draw.circle(window, (eat.color, eat.color, eat.color), (sx, sy), int(eat.radius * scale))
